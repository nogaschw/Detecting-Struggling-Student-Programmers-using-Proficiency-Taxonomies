...
base_path = '/home/nogaschw/Codeworkout/UseData/'
...
class DatasetOneLoss(Dataset):
        ...
        __getitem__(idx):
            ...
            Returns:
                dict: A dictionary containing the following keys:
                    ...
                    - 'task_num' (torch.Tensor):  Tensor containing the number of tasks the student did.
    ...
    def __getitem__(self, idx):
        ...
        len_snapshots = [1 for i in range(self.padding_size_q)]
        ...
        len_tasks = len(row['prev_labels'])
        row['prev_labels'].extend([False for i in range(self.padding_size_q - len(row['prev_labels']))])
        return {
            'text_input_ids': text_inputs['input_ids'].squeeze(0),
            'text_attention_mask': text_inputs['attention_mask'].squeeze(0),
            'code_embedding': embedding,
            'code_num': torch.tensor(len_snapshots),
            'task_num': torch.tensor(len_tasks),
...
class DatasetPTMBoth(DatasetPTM):
    """
    A custom PyTorch Dataset class for embedding questions and code samples for the ablation and base for out PTM
    Attributes:
        df (pd.DataFrame): DataFrame containing the dataset.
    ...
    Methods:
        ...
        __getitem__(idx):
            ...
            Args:
                idx (int): Index of the sample to retrieve.
            Returns:
                dict:- 'dataset': 0 or 1 - 0 for codeworkout or 1 for falcon
    """
    def __init__(self, df, text_tokenizer, max_len_code=1024, padding_size_code=2000, padding_size_q=200):
        super().__init__(df, text_tokenizer, max_len_code, padding_size_code, padding_size_q)
    ...
    def __getitem__(self, idx):
        dict = super().__getitem__(idx)
        row = self.df.iloc[idx]    
        dict['dataset'] = torch.tensor(row['course_id'])
        return dict