...
def load_ids(filepath_prefix='Data/split_ids'):
    ...
    """
    with open(f'{filepath_prefix}train_ids.pkl', 'rb') as f:
        train_ids = pickle.load(f)
    with open(f'{filepath_prefix}valid_ids.pkl', 'rb') as f:
        valid_ids = pickle.load(f)
    with open(f'{filepath_prefix}test_ids.pkl', 'rb') as f:
        test_ids = pickle.load(f)
    return train_ids, valid_ids, test_ids

def create_data_loader(df, dataset, text_tokenizer=None, batch_size=8, max_len_code=512,
                       ids_filepath_prefix='/home/nogaschw/codeworkout/Thesis/split_ids', 
                       padding_size_code=100, padding_size_q=30, seed=42, create_split=False):
    """
    ...
    # Dataset
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, worker_init_fn=lambda worker_id: np.random.seed(seed))
...
def create_data_loader_k_fold(df, dataset, text_tokenizer=None, batch_size=8, max_len_code=1024,
                                padding_size_code=2200, padding_size_q=200, seed=42, k=5):
    """
