...
config = Config()
d = ["codeworkout", "falcon"][config.dataset]
...
# Load the dataset
data = [Codeworkout, Falcon][config.dataset](latents=latents, similarity=False, gemma_path=config.gemma_path)
df = data.df
...
data_loaders = create_data_loader_k_fold(df, dataset, text_tokenizer,max_len_code=config.max_len_code, 
                                         padding_size_q=config.number_coding_tasks, padding_size_code=config.padding_size_code, seed=seed,
...
for fold, (train_dataloader, test_dataloader) in enumerate(data_loaders):
    ...
    for epoch in range(config.epoch):
        ...
        wandb.log({'train_loss': (total_loss / len(train_dataloader)), 'Epoch':epoch, 
                    'total_binary_loss': helperconfig.total_binary_loss, 'total_skills_loss': helperconfig.total_skills_loss})
...
for metric, avg in avg_results.items():
    print(f"{metric}: {avg:.4f}", flush=True)
