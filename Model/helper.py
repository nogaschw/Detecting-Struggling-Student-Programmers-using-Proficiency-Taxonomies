...
def training_loop(model, train_dataloader, test_dataloader, optimizer, criterion, device, name, caculate_func=caculate_2losses, use_wandb=True):
    """
    ...
    for epoch in range(100):
        ...
        skills_loss = helperconfig.total_skills_loss
        binary_loss = helperconfig.total_binary_loss
        helperconfig.total_skills_loss = 0
        helperconfig.total_binary_loss = 0
        ...
        if use_wandb:
            wandb.log({
                ...
                "skills_loss": skills_loss,
                "binary_loss": binary_loss,
                "skills_loss_test": helperconfig.total_skills_loss,
                "binary_loss_test": helperconfig.total_binary_loss,                
                "patience": patience
            })
...
def results(threshold, y_true, y_prob, use_wandb=True):
    """
    ...
    if use_wandb:
        wandb.log({
            ...
            "roc_auc": roc_auc,
            "accuracy": accuracy,
            f"precision_{best}": precision,
            f"recall_{best}": recall,
            f"f1_{best}": f1,
            f"confusion_matrix_{best}": cm
        })
