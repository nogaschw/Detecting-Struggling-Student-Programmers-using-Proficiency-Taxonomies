...
class TBPP_Generation(nn.Module):
    def __init__(self, text_model_name, code_len, num_skills=10):
        ...
        self.norm_lstm_output_prev_labels = nn.BatchNorm1d(config.snapshots_hl + 1) # +1 for prev_struggling

        self.lstm_submissions = nn.LSTM(
           config.snapshots_hl + 1,  # +1 for prev_struggling
           config.submission_hl,
           batch_first=True,
           bidirectional=False,
        )

        self.skills_rep = nn.Linear(config.submission_hl, num_skills)
    ...
    def _process_submissions_sequence(self, task_submissions, task_num):
        ...
        """
        batch_size, q_num, len = task_submissions.size()
...
class AblationStudy(TBPP_Generation):
    def __init__(self, sequence_len, text_model_name, code_len, num_prog_concepts=10, dropout_rate=0.3):
        """
        ...
        """
        super(AblationStudy, self).__init__(text_model_name, code_len, num_skills=num_prog_concepts)
    ...
    def forward(self, text_input_ids, text_attention_mask, code_embedding, code_num, task_num, prev_struggling): 
        """
        ...
        """
        text_output, TBPP = self.createTBPP(text_input_ids, text_attention_mask, code_embedding, code_num, task_num, prev_struggling)
...
class PTM(TBPP_Generation):
    def __init__(self, text_model_name, code_len, num_students, num_skills=13, num_prog_concepts=10):
        ...
        """
        global config
        print(num_skills, num_prog_concepts)
        super(PTM, self).__init__(text_model_name, code_len, num_skills=num_prog_concepts)
        ...
        self.cross_attention = nn.MultiheadAttention(embed_dim=input_dim, num_heads=6, dropout=0.1)
