...
def tokenize_vec(tokenizer, samples, max_len, limit_len):
    ...
    return input_ids_stack, attention_mask_stack
...
class DatasetCodeQuestion(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Tokenize code samples
        input_ids_stack,attention_mask_stack = tokenize_vec(self.code_tokenizer, code_samples, self.max_len_code, self.limit_len)
...
class DatasetCodeQPrevQ(DatasetCodeQuestion):
    ...
    def __getitem__(self, idx):
        ...
        for i, c in enumerate(code_samples):
            ...
            if i == self.limit_len - 1:
                break

        # Tokenize prev questions
        question_input_ids_stack, question_attention_mask_stack = tokenize_vec(self.text_tokenizer, prev_question, self.max_len_text, self.limit_len)
...
def oversampling_df(df, ratio=4):
    struggling_df = df[df['struggling'] == 1]
    not_struggling_df = df[df['struggling'] == 0]  
...
def create_data_loader(df, dataset, text_tokenizer=None, code_tokenizer=None, batch_size=8, balanced_def=undersampling_df, ratio=4,
    ...
    valid_dataset = dataset(valid_df, text_tokenizer, code_tokenizer, feature=feature)
    test_dataset = dataset(test_df, text_tokenizer, code_tokenizer, limit_len=limit, feature=feature)
