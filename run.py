...
code_model_name = 'neulab/codebert-java'
path = "/home/nogaschw/Codeworkout/df_taxonomy.csv"
...
def run(df, dataset, model, name, lr, balance_def):
    ...
    # dataset
    train_dataloader, valid_dataloader, test_dataloader = create_data_loader(df, dataset, text_tokenizer, code_tokenizer, batch_size=1, balanced_def=balance_def, feature=feature)
    ...
    lr_scheduler = get_scheduler("linear", optimizer=optimizer, num_warmup_steps=0, num_training_steps=len(train_dataloader)*20,) # avarage of epoch with early stop
...
if __name__ == '__main__':
    ...
    df = get_df()
    balance_def = [oversampling_boosting, oversampling_df, undersampling_df][balance_def]
    ...
    model_choice = [CodeQuestionLSTMModel(text_model_name, code_model_name, hidden_size, num_layers),
                    CodeQPrevQLSTMModel(text_model_name, code_model_name, hidden_size, num_layers, mean=True)][all_questions]
